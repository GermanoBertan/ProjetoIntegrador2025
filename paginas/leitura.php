...
include("./classes/Leitura.php");
    ...
    $stmt = $conexao->prepare("
        SELECT * FROM leitura AS le
            ...
            WHERE codLeitura=? AND fk_usuario=? AND situacao=1
            LIMIT 1;
    ");
...
<div class="container my-4 shadow-lg p-4 bg-white rounded">
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
      <h3 class="mb-0">📚 DETALHES DA LEITURA</h3>
    ...
    <div class="col-12 col-md-8">
      ...
      <div class="recuoLeituraInfoLivro mb-4" style="text-align: -webkit-center;" >
        ...
        <div class="progress mb-2" style="height: 15px; width: 100%; max-width: 100%;">
          <div class="progress-bar corAzul" role="progressbar" style="width: <?=$progressPercent?>%;" aria-valuenow="<?=$progressPercent?>" aria-valuemin="0" aria-valuemax="100">
            <?=$progressPercent?>%
          </div>
        </div>

        <form action="paginas/codigosPHP/atualizarLeitura.php?codLeitura=<?=$dados['codLeitura']?>" method="POST" class="row g-2 align-items-center" style="max-width: 400px;">
            <div class="col-8 col-sm-9">
                <input type="number" name="novaPagina" class="form-control" 
                    min="1" max="<?=$dados['numPagina']?>" 
                    placeholder="Digite a nova página..." 
                    value="<?=$dados['paginaAtual']?>" required>
            </div>
            <div class="col-4 col-sm-3">
                <button type="submit" class="btn corAzul corBranca w-100">
                Atualizar
                </button>
            </div>
        </form>

        <p class="nPagina mt-1">Página atual: <?=$dados['paginaAtual']?> de <?=$dados['numPagina']?></p>
...
<hr>
    ...
    <div class="recuoLeituraInfoLivro mb-4" >
        ...
        <p class="fonteCinza  mt-1 mb-1 m-2">Mova ou finalize sua leitura. Ao mover a leitura para Quero ler, você perderá a página atual e a nota.</p>  

        <div class="d-flex">

        <!-- Mover para 'Quero Ler' (limpa dados de leitura/resenha/nota) -->
        <form action="paginas/codigosPHP/moverQueroLer.php" method="POST" class="me-2">
            <input type="hidden" name="codLeitura" value="<?= $dados['codLeitura'] ?>">
            <button type="submit" name="moverQueroLer" title="Mover a leitura para Quero Ler (Excluirá os dados de sua leitura)" class="mt-1 mb-1 m-2 btn btn-sm btn-warning">
              Quero Ler
            </button>
          </form>

        <!-- Finalizar leitura (somente se página atual == total) -->
        <form action="paginas/codigosPHP/finalizarLeitura.php" method="POST" >
          <input type="hidden" name="codLeitura" value="<?= $dados['codLeitura'] ?>">
          <button type="submit" name="finalizarLeitura" title="Mover leitura para Lido" class=" mt-1 mb-1 m-2 btn btn-sm btn-success">
            Finalizar
          </button>
        </form>
      </div>
...
<?php
    ...
    echo 'Essa leitura não pertence a você! <br> <a href="index.php?menu=inicio">Voltar a página inicial.</a>';
}
?>